...
const menuItems = computed(() => {
  if (userStore.isAuthenticated) {
    return [
      {
        label: userStore.user.firstname + " " + userStore.user.lastname, icon: "pi pi-user",
        items: [{
          label: "Déconnexion",
          icon: "pi pi-sign-out",
          url: "/web/user/logout",
...
<template>
  <div v-if="userStore.isAuthenticated">
    <Avatar :label="userStore.user.firstname[0] + userStore.user.lastname[0]" class="mr-2 cursor-pointer"
